...
"hello world" is a character and R recognizes characters only if there are quotes around it. Characters are one of R's four basic data types. Let's go through them.
...
A character variable is used to represent letters, codes, or words. Character values are often referred to as a “string”. You might think that the value "1" is a number. Well, with quotes around, it isn’t! Anything with quotes will be interpreted as a character. No ifs, ands or buts about it.
...
Other common data structures are matrices and lists.  We won't go through them in depth here as we won't encounter them too much in this class, but you can learn more about them [here](https://rspatial.org/intr/3-basic-data-structures.html).
...
We won't run into lists too much in this class, only encountering them when we deal with spatial data.
...
Page 228 in DSR provides more examples of writing your own functions.
...
In order to use functions in a new package, you first need to install the package using the `install.packages()` command. For example, we will be using commands from the package **tidyverse** throughout the rest of this course. 
...
After you've done that, answer the homework questions that are uploaded on Canvas.  Submit an R Markdown and its knitted document on Canvas by the assignment due date.
