...
The YAML header controls how R Markdown renders your `.Rmd` file. A YAML header is a section of key:value pairs surrounded by - - - marks.  It is located at the top of your R Markdown document.

<br>

In the R Markdown file you created, the YAML should already have "Assignment 1" next to "title:"  and your name next to "author:".  The current date should be next to "date:". The option  "output:" tells R which file format you want your knitted document in.  The default is html, so you should see "html_document".  For a pdf, you should have "pdf_document"  and for word it will be "word_document". There are a bunch of other features you can adjust in your YAML, but these four items are what you need in the YAML at the minimum. [Here](https://bookdown.org/yihui/rmarkdown/html-document.html) is one source for explaining what other YAML options are available.  Your YAML might generally look like the following (I added a subtitle to indicate our class).
...
Assignments will typically ask you to write R code to accomplish a data analysis task. You present and execute your R code inside R code chunks.  You'll find three R code chunks already in your R Markdown menu - these are automatically placed in every new R Markdown file to help the new user understand how R Markdown files are structured.  

R code is inserted in between `` ```{r} `` and `` ``` ``.  For example, let's say we ask you to use R code to add `1 + 1` and show the result. To designate `1+1` as R code, it will look like the following in your R Markdown document.
...
When answering an assignment question, you'll have the following sequence of components in your R Markdown document: Question, R code answering the question, and your text to explain the results.  The question and text explaining the result reside outside of the R chunk. Let's say you see this in one of your assignments
...
Run that code chunk to make sure it works (you should get 2!).  *Then* proceed to the next question.  Let me emphasize: **Do not write all of your code answering every question in the assignment and run it at the very end.** TEST, Test, and test your code to make sure it runs properly.
...
To Knit your document click ![](/Users/noli/Documents/UCD/teaching/GEO 200CN/lab/geo200cn.github.io/hwguide6.png), which will be located at the top of the upper left R Markdown window.  Note that you can select your document type when knitting by clicking the pull down menu next to ![](/Users/noli/Documents/UCD/teaching/GEO 200CN/lab/geo200cn.github.io/hwguide6.png) and selecting your document choice (default is html).  When you start knitting, you will notice that a new window on the bottom left will appear in place of the console.  The window will show the progress in your knitting.  R is going through each R code chunk one at a time. The percentages you will see will be based on the proportion of your R Markdown file that R has successfully knitted. See Figure below.
...
3. Create an R Markdown for your assignment and save it in the appropriate assignment folder on your hard drive.
4. Download any data needed for the assignment into the same folder.  For most assignments, I will upload the data on GitHub, which you can directly link to in R, so you won't have to download data.
...
* Most of the questions will ask you to run code.  Show that code in R Markdown chunks. Bottom line: Any code you used to get a result should be in your assignment. Otherwise, you will get points off, and for some questions, get *all* points off.
  + Comment on what the code is doing within the R chunk using text after `#`
  + Break up your code into different chunks where it makes sense. For some questions, you might include all code for a question in one single chunk.  For other questions, you might break up the code into several chunks. Name each chunk so that if you encounter an error when knitting, you will know which chunk is giving you problems.
...
6. After you've completed the first question, knit to an html (preferably) or pdf file. Make sure it knits properly. If it does not, examine the error, and fix the problem.
...
8. Once you've completed all questions and successfully knitted, submit the `.Rmd` and knitted files on Canvas **before** the designated due time.
9. Smile, pat yourself on the back, and have some ice cream (I like these [folks](https://threetwinsicecream.com/)).
