...
In this lab, you will learn how to use **tidyverse** functions to accomplish various data transformation tasks. Data transformation is the process of transforming raw data into a more usable form.  It is relatively rare outside of the cloistered setting of a University classroom to have a clean data set ready for analysis. For example, you might want to analyze just Yolo county neighborhoods. Or you might want to discard certain variables from the dataset to reduce clutter. Or you encounter missing data. Some [estimate](https://msimonline.ischool.uw.edu/blog/what-is-data-wrangling/) that data cleaning can take upwards of 80% to 90% of your data analysis time. 

We will rely on **tidyverse** to demonstrate data transformation in R because its consistency, together with the goal of increasing productivity, mean that the syntax of tidy functions is typically straightforward to learn.  Because of time constraints, we will not be able to go through all of **tidyverse**'s data transformation functions in this guide.  Some you will encounter in other course lab guides. Others you may need to learn on your own.  The best resource is the book [R for Data Science](http://r4ds.had.co.nz/index.html) (RDS), which is free online and was written by **tidyverse**'s creators.  You can also go straight to the **tidyverse** official [website](https://www.tidyverse.org/). You can also check out RStudio's [cheat sheets](https://posit.co/resources/cheatsheets/).
...
All of **tidyverse**'s functions are located in the package **tidyverse**, which you should have already installed in last lab. Remember that you will install packages once and never more (unless you update R).  But, you need to always load the package in your current session using `library()`
...
Let's bring in some data to help illustrate data transformation.  I uploaded a file on GitHub containing the number of Hispanic, white, Asian, and black residents in 2018 in California neighborhoods taken from the United States [American Community Survey](https://www.census.gov/programs-surveys/acs).   To import this file in R, use the **tidyverse** function `read_csv()` command.
...
Note the underscore in between `read` and `csv` in the code above.  There is a base R function called `read.csv()`.  What is the difference? The function `read.csv()` stores your data into a regular data frame. The function `read_csv()` stores your data in a special tidy R object - a tibble.
...
One of the clear advantages of tidy R, at least when you are first learning R, is its assortment of user-friendly data transformation functions, which is a part of its **dplyr** package. Compared to base R, the beauty of these **dplyr** functions is that they feature consistent design principles and easily work with non-standard evaluation (i.e., you don’t have to put quotes around variable names).

In the rest of this guide, we'll go through some of the basic data transformation techniques using the functions found in the package **dplyr**, which was automatically installed and loaded when you brought in the **tidyverse** package.  These functions can be used for tibbles and regular data frames.
...
Let's join a dataset containing some health and air pollution data from California's [CalEnviroScreen](https://oehha.ca.gov/calenviroscreen/report/calenviroscreen-40) program. Read it in from GitHub using our new friend `read_csv()`.
...
What if you want to arrange rows? Use the `arrange()` function.  Arranging or sorting data refers to the process of ordering rows numerically or alphabetically in a data frame or table by the values of one or more variables. Sorting can make it easier to visually scan raw data, such as for the purposes of identifying extreme or outlier values. 

Use the function `head()` to see what are the observations at the top of your file
...
What if we wanted to sort the rows by population size?
...
How would you go about sorting the data set by population size from largest to smallest?
...
To get some practice on data transformation, answer the homework questions that are uploaded on Canvas in an RMarkdown file. Submit the R Markdown and its knitted document on Canvas by the assignment due date.
