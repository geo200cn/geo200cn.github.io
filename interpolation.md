...
Last week, we learned about the prediction framework. In this lab guide, we apply this framework to the spatial world. Spatial interpolation is the prediction of exact values of attributes at unsampled locations from measurements made at control points in the same study area. We will learn how to run the deterministic spatial interpolation methods described in OSU Ch. 9. The objectives of this lab are as follows
...
To help us accomplish these learning objectives, we will use precipitation data from weather stations in California. Our goal is to create a raster surface of California total precipitation values. This week’s assignment questions on spatial interpolation are scattered throughout this lab.
...
We'll be using two new packages in this lab.  Install the package.


``` r
install.packages(c("gstat", "rspat"))
```
...
To demonstrate spatial interpolation we'll be relying on a dataset containing monthly precipitation data for weather stations in California. Bring in the dataset *precipitation* which is a part of the **rspat** package. We can bring it directly into R using the function `spat_data()`.
...
## 1 280058.6 -167265.5 ID741         DEATH VALLEY -59  7.4 9.5 7.5 3.4 1.7 1.0
...
```
## [1] 330.0235 336.7008 314.5188 321.3066 313.4140
```
